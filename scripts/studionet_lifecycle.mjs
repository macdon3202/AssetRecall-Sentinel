...
const address=process.env.ASSET_RECALL_ADDRESS??'0xda6B9aD1A9541b76dd542ca99a1bbC11E387b784';
...
const stateDir=new URL('./.state/',import.meta.url);mkdirSync(stateDir,{recursive:true});const stateFile=new URL('./.state/studionet-v2.json',import.meta.url);const enc=x=>JSON.stringify(x,(_,v)=>typeof v==='bigint'?String(v):v,2);let state=existsSync(stateFile)?JSON.parse(readFileSync(stateFile)):{address,fixture:{metadata,digest},actions:{}};const save=()=>writeFileSync(stateFile,enc(state));
...
const action=process.argv[2]??'read',initial=await snapshot();if(initial.config.version!=='ASSET_RECALL_SENTINEL_V2')throw Error('WRONG_VERSION');
...
else if(action==='evaluate-refresh-recovery')await send('evaluate-refresh-recovery','A','evaluate_scan',[1n]);
